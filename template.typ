...
#let abbreviations(..items) = {
  ...
  // Выводим заголовок раздела (жирный, по левому краю)
  align(left)[
...
// Функция для вывода Содержания по вашему стандарту
#let contents() = {
  // Выравниваем слово "Содержание" по центру и делаем жирным
  align(center)[
    #strong[Содержание]
  ]
  
  // Задаем вертикальный отступ
  v(24pt)

  // Выводим само оглавление
  outline(
    title: none,
    indent: 1.25cm,
    depth: 2 
  ...
  // Автоматический перенос на следующую страницу после оглавления
  pagebreak(weak: true)
}
